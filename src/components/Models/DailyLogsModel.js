...
export default class DailyLogsModel extends Model {
    ...
    static parseDailyLogsModelFromJSON(json) {
        ...
        for(var jsonItem in json) {
            ...
            let itemFromJson = new DailyLogsModel(json[jsonItem],json.length,title);
            dailyLogsList.push(itemFromJson);
        }

        if(dailyLogsList.find(k=>k._is_manual=="2"&&k._status=="101")===undefined )
                        {
                            itemFromJson = new DailyLogsModel({added_date: json[0].added_date,
