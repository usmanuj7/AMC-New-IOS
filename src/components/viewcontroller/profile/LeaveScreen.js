...
export default class LeaveScreen extends React.Component {
    ...
    if (Utilities.isValidString(this.state.leaveTypeLabel)) {
      if (Utilities.isValidString(this.state.checkedStartDate)) {
        if (Utilities.isValidString(this.state.checkedEndDate)) {
          ...
          if (
            this.state.leaveTypeId === '1' ||
            this.state.leaveTypeId === '2' ||
            this.state.leaveTypeId === '3'
          ) {
            ...
            this.WebServicesManager.postMethodApplyLeave(
              {dataToInsert: leaveData, apiEndPoint: 'apply_leave'},
              (statusCode, response) => {
                if (Utilities.checkAPICallStatus(response.responseCode)) {
                  if (
                    response.response !== undefined &&
                    response.response.status === 'false_leave_exceeded'
                  ) {
                    this.setState({isLoadingIndicator: false});
                    this.dropDownAlertRef.alertWithType(
                      'info',
                      'Info',
                      response.response.err_mesg,
                    );
                  } else {
                    this.setState({isLoadingIndicator: false});
                    this.setState({
                    ...
                    this.dropDownAlertRef.alertWithType(
                      'info',
                      'Success',
                      'You have successfully logged your leave',
                    );
                  }
                } else if (statusCode === 400) {
                  this.setState({isLoadingIndicator: false});
  ...
  async setofflineDataLeave(appliedLeave) {
    ...
    if (appliedLeavePrevArray !== null) {
      ...
      this.dropDownAlertRef.alertWithType(
        'info',
        'Success',
        'Leave request applied on offline mode',
      );
    } else {
  ...
  render() {
    if (this.state.checkedStartDate !== '') {
      var startDate =
        moment(this.state.checkedStartDate).format('dddd') +
        ', ' +
              ...
              <View style={{flex: 0.2, paddingTop: 10, marginRight: 20}}>
                <View style={{alignItems: 'flex-end', marginRight: 20}}>
                  <Button
                    medium
                    onPress={() =>
                      this.props.navigation.navigate('LeaveHistoryScreen')
                    }
                    style={{
                      width: 150,
                ...
                <View style={styles.transparentInputBoxTextArea}>
                  <TextInput
                    ...
                    onChangeText={searchString => {
                      this.setState({comments: searchString});
                    }}
                    underlineColorAndroid="transparent"
                  />
                </View>
