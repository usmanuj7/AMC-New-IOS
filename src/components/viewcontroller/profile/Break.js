   ...
   async handleStartBreak()
    {
      ...
      var today = moment(new Date());
        ...
        var attendence = { title:"StartBreak", staffid: this.state.profileDataSurce._staffid,attendance_id:dailyLogsModelDataSource[0]._attendance_id,
        status:"101",swipe_time:new Date().getHours() +':'+new Date().getMinutes()+':'+new Date().getSeconds(),clock_date:
        moment(new Date()).format("YYYY-MM-DD") };
                this.WebServicesManager.postMethodStartBreak({ dataToInsert: attendence, apiEndPoint: "add_break_start" },
                (statusCode, response) => {
                  if (Utilities.checkAPICallStatus(response.responseCode)) {
                    var attendanceData = { title:"StartBreak", staffid: this.state.profileDataSurce._staffid,attendance_id:"",
                    status:"101",swipe_time:new Date().getHours() +':'+new Date().getMinutes()+':'+new Date().getSeconds(),clock_date:
                    moment(new Date()).format("YYYY-MM-DD") };
                    ...
                    Utilities.saveToStorage("startBreakTimeToday", attendanceData).then((value) => {
                ...
                {
                  var attendence = { title:"StartBreak", staffid: this.state.profileDataSurce._staffid,attendance_id:"",
              status:"101",swipe_time:new Date().getHours() +':'+new Date().getMinutes()+':'+new Date().getSeconds(),clock_date:
              moment(new Date()).format("YYYY-MM-DD") };
  ...
  async  handleEndDuty()
    {
      ...
      var today = moment(new Date());
        ...
        var attendence = { staffid: this.state.profileDataSurce._staffid,attendance_id:dailyLogsModelDataSource[0]._attendance_id,
            clock_out:new Date().getHours() +':'+new Date().getMinutes()+':'+new Date().getSeconds(),
            date_times:moment(new Date()).format("YYYY-MM-DD") };
            
        this.WebServicesManager.postMethodEndDuty({ dataToInsert: attendence, apiEndPoint: "add_clock_out" },
          (statusCode, response) => {
            if (Utilities.checkAPICallStatus(response.responseCode)) {
              var attendanceData = {
                title: "EndDuty",staffid: this.state.profileDataSurce._staffid,attendance_id:"",
                clock_out:new Date().getHours() +':'+new Date().getMinutes()+':'+new Date().getSeconds(),
                date:moment(new Date()).format("YYYY-MM-DD") 
              };
              this.setofflineDataEndDuty(attendanceData)
              var totalHours=response.response.total_hours;
              Utilities.saveToStorage("totalHoursWorked", JSON.stringify(totalHours));
      ...
      this.WebServicesManager.postApiDailyAttendence({ dataToInsert: Leave, apiEndPoint: "get_daily_attendance_log" },
          ...
          else if(statusCode===400)
          {
              ...
              var attendence = {title:"EndDuty", staffid: this.state.profileDataSurce._staffid,attendance_id:"",
                clock_out:new Date().getHours() +':'+new Date().getMinutes()+':'+new Date().getSeconds(),
                date:moment(new Date()).format("YYYY-MM-DD") };
                ...
                <Text style={styles.HeaderTextTitleSemiBold}>{this.state.startDutyTime}</Text>
                    ...
                    <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
                       ...
                       <View style={{flexDirection:'row', marginTop:20}}> 
                        ...
                        <Image source={require('../../../ImageAssets/cup.png')} style={{ width: 20, height: 20,padding:5,margin:10 }} />
                            <Text style={styles.buttonTextSmall}>
                                Start Break
                             </Text>
                        </Button>
                        <Button onPress={() => this.handleEndDuty()} block style={{ flexDirection:'row',flex:1,marginRight:20, borderRadius: 7, backgroundColor: constants.colorRed9d0000, height: 45 }}>
                        <Image source={require('../../../ImageAssets/end.png')} style={{ width: 20, height: 20, marginRight:10 }} />
                            <Text style={styles.buttonTextSmall}>
