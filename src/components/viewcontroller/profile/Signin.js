  ...
  async checkNotif(dailyLogsModelDataSource,prevDate,profileData)
{
        ...
        if(dailyLogsModelDataSource.find(k=>k._is_manual=="2"&&k._status=="101")===undefined )
                {
                  ...
                  if (notificationsArrayData !== null) {      
                    ...
                    if(!notificationsArray.includes(dailyLogsModelDataSource[0]._clock_date))
      ...
      this.WebServicesManager.postApiCallAttendence({ dataToInsert: staffData, apiEndPoint: "get_staff_weekends" },
          (statusCode, response) => {
              if (Utilities.checkAPICallStatus(statusCode)) {
                ...
                {
                var weekDayName =  moment(this.state.dailyLogsModelDataSource[0]._clock_date).format('dddd');
  ...
  async  Login() {
    if (Utilities.ValidateEmail(this.state.userEmail)) {
          ...
          if (Utilities.checkAPICallStatus(statusCode)) {
            if (response.responseCode !== 204 && response.responseCode !== 404) {
              ...
              AsyncStorage.setItem('profileData', JSON.stringify(profileData)).then((value) => {
                this.setState({ isLoadingIndicator: false })
                this.dropDownAlertRef.alertWithType('success', 'Success', "You have successfully logged in");
                var date = new Date();
                ...
                (statusCode, response) => {
                    if (Utilities.checkAPICallStatus(statusCode)) {
                        ...
                        Utilities.saveToStorage("leaveTypeData",JSON.stringify(leaveTypeData));
                        ...
                        this.WebServicesManager.postApiLeaveHistory({ dataToInsert: Leave, apiEndPoint: "get_leave_data" },
                            (statusCode, response) => {
                                if (Utilities.checkAPICallStatus(statusCode)) {
                                  ...
                                  Utilities.saveToStorage("leaveHistoryData",JSON.stringify(leaveModel));
            ...
            this.setState({ isLoadingIndicator: false })
            this.dropDownAlertRef1.alertWithType('info', 'Error', "Unable to conenct to internet");
          }
          else {
            this.setState({ isLoadingIndicator: false })
      ...
      this.dropDownAlertRef1.alertWithType('info', 'Error', "Please enter valid Email");
  }
render() {

  return (
   
    <ImageBackground style={{height:'100%'}} source={require('../../../ImageAssets/background.png')}
      ...
      <View style={styles.transparentInputBox}>
        <Ionicons style={styles.searchIcon} name="md-lock" size={24} color="#000" />
        <TextInput
          style={styles.inputNotCenterAligned}
      ...
      <Button onPress={() => this.Login()} block style={{ margin: 18, borderRadius: 7, backgroundColor: constants.colorRed9d0000, height: 40 }}>
