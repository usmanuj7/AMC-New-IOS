...
export default class MonthScreen extends React.Component {
    ...
    updateData(monthdate) {
       ...
       var startDate =monthdate.split(" ")[1] + '-' + moment().month(monthdate.split(" ")[0]).format("MM") + '-' + "01" + ' 00:00:00';
       var month_year = monthdate.split(" ")[1] + "-" + moment().month(monthdate.split(" ")[0]).format("MM");
       this.setState({ month: monthdate,monthDate:startDate});
        var attendence = { staffid: this.state.profileDataSurce._staffid, month_year: month_year };
    ...
    async componentWillMount() {
        ...
        var Months = moment(this.state.checkedDate).format("MMMM YYYY");
        this.setState({
        ...
        if (profile !== null) {
            ...
            this.updateData(moment(new Date()).format("MMMM YYYY"))
    ...
    Next() {
        ...
        var nextDate = moment(this.state.monthDate, 'YYYY-MM-DD hh:mm:ss').add(1, 'M');
        var nextMonth = moment(this.state.monthDate, 'YYYY-MM-DD hh:mm:ss').add(1, "month").format("MMMM YYYY");
        this.setState({ month: nextMonth, monthDate: nextDate });
        this.updateData(nextMonth);
    }

    Prev() {
        var prevDate = moment(this.state.monthDate, 'YYYY-MM-DD hh:mm:ss').subtract(1, 'M');
        var prevMonth = moment(this.state.monthDate, 'YYYY-MM-DD hh:mm:ss').subtract(1, "month").format("MMMM YYYY");
    ...
    render() {
        const chartsData = [parseInt(this.state.totalWorkedMinutes), parseInt(this.state.totalBreakMinutes), parseInt(this.state.totalMinutes)]
            ...
            <Container>
                ...
                <ImageBackground source={require('../../../ImageAssets/background.png')}
                    ...
                    <View style={{ backgroundColor: '#595278', flexDirection: 'row' }}>
                        <Button onPress={() => this.props.navigation.navigate("ReportScreen")} style={{ flex: 1, justifyContent: 'center', backgroundColor: constants.colorPurpleLight595278, }}>
                            <Text style={{ color: constants.colorWhitefcfcfc, }}>By Week</Text>
                        </Button>
                        <Button onPress={() => this.props.navigation.navigate("MonthScreen1")} style={{ flex: 1, justifyContent: 'center', backgroundColor: constants.colorPurpleDark302757, opacity: 0.4, }}>
                    ...
                    <View style={{  flexDirection: 'row', backgroundColor: '#595278', justifyContent: 'center', alignItems: 'center' }}>
                        ...
                        <TouchableOpacity onPress={() => this.props.navigation.navigate("SelectMonthScreen", { context: this })}>
