...
export default class ActionTimePick extends React.Component {
    ...
    componentWillMount() {
        ...
        this.setState({clocktimeToCheck:this.props.navigation.state.params.selectedItem._clock_time});
        this.setState({indexofSelectedHours:this.props.navigation.state.params.selectedItem._clock_time.split(" ")[1].split(":")[0]});
        this.setState({indexofSelectedMinutes:this.props.navigation.state.params.selectedItem._clock_time.split(" ")[1].split(":")[1]});

         
        BackHandler.removeEventListener('hardwareBackPress', this.handleBackButton);
        var Leave = {
            staffid: this.props.navigation.state.params.selectedItem._staffid,
            clock_date: moment(this.props.navigation.state.params.selectedItem._clock_date).format('YYYY-MM-DD')
        };
   ...
   async saveToStorage(attendanceData)
    {
        ...
        setTimeout(() => {this.props.navigation.navigate("CalanderScreen");}, 3000);
    }
    ...
    async Save() {
        ...
        var profile = {
            ...
            time: this.state.indexofSelectedHours + ":" + this.state.indexofSelectedMinutes + ":" + "00",
        ...
        this.WebServicesManager.postMethodUpdateTime({ dataToInsert: profile, apiEndPoint: "update_time" },
            (statusCode, response) => {
                if (Utilities.checkAPICallStatus(statusCode)) {
                    this.setState({ isLoadingIndicator: false });
                    this.dropDownAlertRef.alertWithType('info', 'Alert', "Record is updated successfully");
                  ...
                  {  for (let index = 0; index < todayAttemArray.length; index++) {
                        ...
                        && todayAttemArray[index].date === (selectedItemTemp._clock_date)) {
                            ...
                            var attendanceData = {
                                title: "StartDuty", date: this.props.navigation.state.params.selectedItem._clock_date,
                                 staffid: this.props.navigation.state.params.selectedItem._staffid, clock_in:  this.state.indexofSelectedHours + ":" + this.state.indexofSelectedMinutes + ":" + "00"
                              };
                    ...
                    {
                    if (todayAttemArray.hasOwnProperty(index) && todayAttemArray[index].title === (selectedItemTemp._title).replace(/\s+/g, '')
                    && todayAttemArray[index].clock_date === (selectedItemTemp._clock_date)) {
                ...
                else
                {
                    ...
                    if(selectedItemTemp._title==="Start Duty" && moment(new Date()).format('YYYY-MM-DD')  === (selectedItemTemp._clock_date))
                        {
                            var attendanceData = {
                                title: "StartDuty", date: this.props.navigation.state.params.selectedItem._clock_date,
                                 staffid: this.props.navigation.state.params.selectedItem._staffid, clock_in:  this.state.indexofSelectedHours + ":" + this.state.indexofSelectedMinutes + ":" + "00"
                    ...
                    if(selectedItemTemp._title==="Start Break"&& moment(new Date()).format('YYYY-MM-DD')  === (selectedItemTemp._clock_date))
                    {
                        var attendanceData = {
                            title: "StartBreak", clock_date: this.props.navigation.state.params.selectedItem._clock_date,
                            status:101, staffid: this.props.navigation.state.params.selectedItem._staffid, swipe_time:  this.state.indexofSelectedHours + ":" + this.state.indexofSelectedMinutes + ":" + "00"
                          };
                          Utilities.saveToStorage("todayTime", attendanceData);
                          setTimeout(() => {this.props.navigation.navigate("CalanderScreen");}, 3000);
                }
                if(selectedItemTemp._title==="End Break"&& moment(new Date()).format('YYYY-MM-DD')  === (selectedItemTemp._clock_date))
                {
                    var attendanceData = {
                        title: "EndBreak", clock_date: this.props.navigation.state.params.selectedItem._clock_date,
                        status:101, staffid: this.props.navigation.state.params.selectedItem._staffid, swipe_time:  this.state.indexofSelectedHours + ":" + this.state.indexofSelectedMinutes + ":" + "00"
                      };
                      Utilities.saveToStorage("todayTime", attendanceData);
                      setTimeout(() => {this.props.navigation.navigate("CalanderScreen");}, 3000);
    ...
    _handleselectedMinutes = (index) => { this.setState({ indexofSelectedMinutes: index }); }

    renderItem1(item1) {
   
        if (Utilities.isValidString(this.state.clocktimeToCheck)&&this.state.clocktimeToCheck.split(" ")[1].split(":")[0] == item1.item.title) {
            return (
                <TouchableOpacity onPress={() => this._handleselectedHours(item1.index)}
            ...
            return (
                <TouchableOpacity onPress={() => this._handleselectedHours(item1.index)}
                    style={this.state.indexofSelectedHours === item1.index ?
                        styles.selected : styles.rowFrontforTimePicker}>
                    <View style={{ flex: 1, flexDirection: 'column', }}>
                        <View style={{ alignItems: 'flex-end', justifyContent: 'flex-end' }}>
                            <Text style={this.state.indexofSelectedHours === item1.index ?
                        styles.selectedText : styles.unSelectedText} >{item1.item.title}</Text>
                        </View>
                    </View>
                </TouchableOpacity>
            )
    ...
    renderItem2(item2) {
        ...
        if (Utilities.isValidString(this.state.clocktimeToCheck)&&this.state.clocktimeToCheck.split(" ")[1].split(":")[1] == item2.item.title) {
            return (
            <TouchableOpacity onPress={() => this._handleselectedMinutes(item2.index)}
                style={styles.selected}>
                ...
                else
                {
                    return (
                        <TouchableOpacity onPress={() => this._handleselectedMinutes(item2.index)}
                            style={this.state.indexofSelectedMinutes === item2.index ?
                                styles.selected : styles.rowFrontforTimePicker}>
                            <View style={{ flex: 0.5, flexDirection: 'column', }}>
                            <Text style={this.state.indexofSelectedMinutes === item2.index ?
                        styles.selectedText : styles.unSelectedText} >{item2.item.title}</Text>
        ...
        return (
            ...
            <Container>
                ...
                <ImageBackground source={require('../../../ImageAssets/background.png')}
                    ...
                    <View style={{ flex: 2, alignContent: 'center', marginTop: 10, marginLeft: 20, marginRight: 20, borderRadius: 5, }}>
                        ...
                        <View style={{ flex: 0.2, marginBottom: 10 }}>
                            <Button onPress={() => this.Save()} style={{ borderRadius: 7, backgroundColor: constants.colorRed9d0000, height: 40, justifyContent: 'center' }}>
                    ...
                    <Footer style={{
                        ...
                        bottom: 0
                            ...
                            <Button onPress={() => this.props.navigation.navigate("ActionTimePick")} vertical style={styles.footerButtonInactive}>
