...
export default class WebServicesManager extends React.Component {
    ...
    baseUrl = 'http://198.72.96.174/~timeattendance/v1/mobile/attendance/endpoint/';
    constructor(props) {
        super(props)
    ...
    callPostMethod(apiURL, params, callback) {
        NetInfo.fetch().then(state => {
            if (state.isConnected === true) {
                ...
                fetch(apiURL + params.apiEndPoint, request).then((response) => response.json())
                    .then((responseJson) => {
                        let statusCode = 200;
    ...
    callPostMethodChangePassword(apiURL, params, callback) {
        NetInfo.fetch().then(state => {
            if (state.isConnected === true) {
                var formdata = new FormData();
                formdata.append("staffid", params.dataToInsert.staffid)
                formdata.append("current_password", params.dataToInsert.current_password)
                formdata.append("new_password", params.dataToInsert.new_password)
                formdata.append("new_password_confirmed", params.dataToInsert.new_password_confirmed)
    ...
    callPostMethodDeleteLeave(apiURL, params, callback) {
        NetInfo.fetch().then(state => {
            if (state.isConnected === true) {
                var formdata = new FormData();
                formdata.append("staffid", params.dataToInsert.staffid)
                formdata.append("leave_request_id", params.dataToInsert.leave_request_id)
    ...
    callPostMethodUpdateTime(apiURL, params, callback) {
        NetInfo.fetch().then(state => {
            if (state.isConnected === true) {
                var formdata = new FormData();
                formdata.append("staff_clocktime_id", params.dataToInsert.staff_clocktime_id)
    ...
    postApiDailyAttendenceBarChart(params, callback) {
        return this.callPostMethodAttendanceBarChart(this.baseUrl, params, callback);
    }
    ...
    static postApiCallDutyStatic(params,callback) {
        ...
        fetch("http://198.72.96.174/~timeattendance/v1/mobile/attendance/endpoint/" + params.apiEndPoint, request).then((response) => response.json())
            .then((responseJson) => {
                let statusCode = 200;
                callback(statusCode,responseJson)
            })
